...
constraints, stats contains the routine \texttt{constrOptim()}. Then
there is \texttt{nlm} which is used for solving nonlinear unconstrained
...
\includegraphics{Optimization_files/figure-latex/opt_unconstr_optim(ize)-1.pdf}
...
\includegraphics{Optimization_files/figure-latex/opt_unconstr_optim(ize)-2.pdf}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{optimize}\NormalTok{(func_2_x, }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, }\DecValTok{3}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\section{Application: Least Squares, Weighted Least
Squares}\label{application-least-squares-weighted-least-squares}
