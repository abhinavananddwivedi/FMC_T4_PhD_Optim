...
> Additionally, for minimizing a function subject to linear inequality constraints, stats contains the routine `constrOptim()`. Then there is `nlm` which is used for solving nonlinear unconstrained minimization problems.

Consider the function $f(x) = 2(x_1-1)^2+5(x_2-3)^2+10$

```{r opt_unconstr_optim(ize)}
...
optim_func_1_x <- optim(c(1,1), func_1_x) #x=(x_1,x_2)=c(1,1)
print(optim_func_1_x)
...
optimize(func_2_x, c(0, 3))

```



# Application: Least Squares, Weighted Least Squares
